...
- **年表**（R5a）: 横軸は現在年を中心とした窓、縦軸は分類の 7 レーン（凡例と同じ順・色）。
  - 窓は初期 ±50 年。ホイールで ±10〜±500 年に変えられる（`timeline.ts` の `TIMELINE_HALF_SPAN`。倍率で変わる）。窓の幅は `AppState.timelineHalfSpan` に持つ（共有したい状態なので。R7 で URL に載せる）。タッチ操作での時間ズームは、まだ無い。
  ...
  - 狭い画面（凡例が折りたたまれる幅）では既定で閉じ、「開く／閉じる」のボタンを出す。
