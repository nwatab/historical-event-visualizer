...
- **年表**（R5a）: 横軸は現在年を中心とした窓、縦軸は分類の 7 レーン（凡例と同じ順・色）。
  ...
  - **地図との違いは1つだけ**: `placeKind` が `"none"` の項目も出す。年表では区別しない。選んだとき、どのズームでも地図に出ない項目（places が空、または場所が国だけの importance 1。`mapFilters.ts` の `neverOnMap`）には、詳細パネルに「地図上の位置は不明」と1行出す。
