...
現在（R1）は、画面下部の年スライダー（紀元前3001年〜2025年）で年を動かすと、その年の前後20年に起きた出来事が地図上にマーカーで表示されます。キーボードの ← / → でも1年ずつ（Shift 併用で10年ずつ）動かせます。データは手書きのサンプル約30件です。マーカーは7分類で色分けされ、左上の凡例にホバー（タッチ端末ではタップ）するとその分類だけが強調されます。
