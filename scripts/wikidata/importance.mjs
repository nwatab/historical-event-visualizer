// importance（1〜3）の試算。純粋関数のみ。R4b-1 ではレポートに使うだけで、アプリには組み込まない。
//
// 規則（R4b-1 の依頼どおり）:
...
/**
 * @param {{ year: number, sitelinks: number, cappedAsEngagement: boolean, vital: boolean }} item
...
export const importanceOf = (item, thresholds) => {
  ...
  const capped = item.cappedAsEngagement ? Math.min(base, 2) : base;
